...
OpenMV-AprilTag 是无人机空中对接项目中的 AprilTag 视觉定位实验仓库。它提供一条基于相机的近距离定位路径，可在对接测试中与 UWB 测距配合使用。

仓库包含 OpenMV 端 AprilTag 检测与位姿输出脚本，也包含 PC 端串口读取和 3D 可视化工具。结构主要面向台架测试、相机标定实验和对接流程集成。
